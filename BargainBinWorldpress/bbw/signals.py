...
def mail_new_posts_to_users(sender, instance, created=False, **kwargs):
    print('=====', sender, str(instance), sep='\n')
    try:
        print(mail_new_posts_to_users.email_initiated)
    except Exception:
    ...
    if sender == Post and created:  # first call of the function (post_save)
        ...
        # Adding people, who subscribed to category to list of recipients.
    ...
    if sender == Post.tags.through and mail_new_posts_to_users.email_initiated and kwargs['action'] == 'post_add':
        ...
        # Ready to send
        # It is implied that splitting this e-mail into len(to_list) separate e-mails is done by mailing service
...
def add_one_to_post_count(sender, instance, **kwargs):
    user = instance.author
    user.total_posts -= 1
    user.save()
